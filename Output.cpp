...
Output::Output()
{
	...
	//water pit and danger zone 
	UI.WaterPitsCellColor = MEDIUMBLUE;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_SET_BELT_CELL] = "images\\belt.jpg";
	MenuItemImages[ITM_SET_WATERPIT_CELL] = "images\\waterpit.jpg";
	MenuItemImages[ITM_SET_DANGERZONE_CELL] = "images\\danger.jpg";
	MenuItemImages[ITM_SET_WORKSHOP_CELL] = "images\\Workshop.jpg";
	MenuItemImages[ITM_SET_ROTATINGGEARcw_CELL] = "images\\gear.jpg";
	MenuItemImages[ITM_SET_ROTATINGGEARacw_CELL] = "images\\gearcounter.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_EXIT_] = "images\\Menu_Exit.jpg";
...
void Output::CreateCommandsBar(Command savedCommands[], int savedCommandsCount, Command availableCommands[], int availableCommandsCount) const
{
	...
	CommandItemImages[MOVE_BACKWARD_ONE_STEP] = "images\\back1.jpg";
	CommandItemImages[MOVE_FORWARD_TWO_STEPS] = "images\\forward2.jpg";
	CommandItemImages[MOVE_BACKWARD_TWO_STEPS] = "images\\back2.jpg";
	CommandItemImages[MOVE_FORWARD_THREE_STEPS] = "images\\forward3.jpg";
	CommandItemImages[MOVE_BACKWARD_THREE_STEPS] = "images\\backward3.jpg";
	CommandItemImages[ROTATE_CLOCKWISE] = "images\\moveclock.jpg";
	CommandItemImages[ROTATE_COUNTERCLOCKWISE] = "images\\movecounter.jpg";
...
// TODO: Draw the player triangle in center(x,y) and filled with the playerColor passed to the function
	...
	DrawTriangle(x, y, triangleHeight, triangleWidth, direction, playerColor, FILLED, 1);
}
...
void Output::DrawBelt(const CellPosition& fromCellPos, const CellPosition& toCellPos) const
{
	...
	if (!fromCellPos.IsValidCell() || !toCellPos.IsValidCell() || fromCellPos.GetCellNum() == 1 || toCellPos.GetCellNum() == 1) {
		return;
	}
	...
	int trianglecenterx = (beltFromCellX + beltToCellX) / 2;
	int trianglecentery = (beltFromCellY + beltToCellY) / 2;

	int triangleWidth = UI.CellWidth / 4;
	int triangleHeight = UI.CellHeight / 4;


	DrawTriangle(trianglecenterx, trianglecentery, triangleHeight, triangleWidth, DIRECTION, UI.BeltColor);
}
...
void Output::DrawFlag(const CellPosition& cellPos) const
{
	...
	if (cellPos.IsValidCell()) {
		...
		int flagPoleStartY = cellStartY + UI.CellHeight / 2;
		int flagPoleEndY = flagPoleStartY + UI.CellHeight / 2;
		pWind->SetPen(WHITE);
		pWind->DrawLine(flagPoleStartX, flagPoleStartY, flagPoleStartX, flagPoleEndY);


		// 		 2. Draw the flag (the triangle)
		int triangleCenterX = flagPoleStartX + UI.FlagPoleWidth / 2 + UI.FlagWidth / 2;
		int triangleTopY = flagPoleStartY - UI.FlagHeight / 2;

		pWind->SetPen(UI.FlagColor);
		pWind->SetBrush(UI.FlagColor);

		DrawTriangle(triangleCenterX, triangleTopY, UI.FlagHeight, UI.FlagWidth, RIGHT, UI.FlagColor, FILLED, 3);
	}
	else { return; }
}
...
void Output::DrawRotatingGear(const CellPosition& cellPos, bool clockwise) const
{
	...
	if (cellPos.IsValidCell())
	{
		...
		if (clockwise)
			DrawImageInCell(cellPos, "images\\gear.jpg", UI.CellWidth / 2, UI.CellHeight / 2);
		else
		{
			DrawImageInCell(cellPos, "images\\gearcounter.jpg", UI.CellWidth / 2, UI.CellHeight / 2);
		}
...
void Output::DrawDangerZone(const CellPosition& cellPos) const
{
	...
	if (cellPos.IsValidCell() && cellPos.GetCellNumFromPosition(cellPos) != 1) {
